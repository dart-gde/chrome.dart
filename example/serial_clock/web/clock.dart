...
const serialport = "/dev/tty.usbserial-AH015USU";
Serial serial;
...
void main() {
  ...
  Serial.ports.then((result) {
    result.forEach((port) {
      if (port == serialport) {
        serial = new Serial(serialport, 115200);
...
class CountDownClock {
  ...
  void updateTime(DateTime now) {
    ...
    if (now.second != displayedSecond) {
      ...
      if (displayedSecond % 5 == 0) {
        serial.write("${now.year}\n").then((_) {});
      } else {
        serial.write("${displayedHour}${displayedMinute}${displayedSecond < 10 ? '0$displayedSecond' : displayedSecond}\n").then((_) {});
      }
    }
  }
  ...
  void setDigits(String digits, List<ClockNumber> numbers) {
    for (int i = 0; i < numbers.length; ++i) {
      int digit = digits.codeUnits[i] - '0'.codeUnits[0];
